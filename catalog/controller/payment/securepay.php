<?php
class ControllerPaymentSecurepay extends Controller {
	public function index() {
		$this->language->load('payment/securepay');

		$data['button_confirm'] = $this->language->get('button_confirm');
		if ($this->config->get('securepay_enviroment') == 'test')
			$data['gateway_url'] = 'https://payment.securepay.com.au/test/v2/invoice';
		else
			$data['gateway_url'] = 'https://payment.securepay.com.au/v2/invoice';
		...
		$return_url = $this->config->get('config_ssl'). 'catalog/controller/payment/securepay_return.php';
		//$this->url->link('payment/securepay/success', '', 'SSL');//'http://demo.vnphpexpert.com/opencart_2/test.php';//
		$notify_url = $this->url->link('payment/securepay/callback', '', 'SSL');
		$cancel_url = $this->config->get('config_ssl'). 'catalog/controller/payment/securepay_return.php';//$this->url->link('checkout/checkout');
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/securepay.tpl')) {
            return $this->load->view($this->config->get('config_template') . '/template/payment/securepay.tpl', $data);
        } else {
            return $this->load->view('default/template/payment/securepay.tpl', $data);
        }
	}

	public function callback() {
		$this->load->model('payment/securepay');
		...
		if($fingerprint_hash == $fingerprint && in_array($rescode, array('00', '08', '11'))) {
			...
			if(in_array($transaction_type, array(0,2,4,6))){
				...
				$securepay_order_id = $this->model_payment_securepay->addOrder($order_info, $txnid);
			} else {
				//auth transaction
				$msg = 'Auth ID: ' . $txnid;
			}
